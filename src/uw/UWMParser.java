 ...
 * @author Aale
 */
public class UWMParser {

    public static void main(String[] args) {
        try {
            SAXBuilder builder = new SAXBuilder();
            String path = "data/uwm.xml";
            Document jdomDocument = builder.build(path);
            Element root = jdomDocument.getRootElement();

            extractElements(root);
        } catch (JDOMException ex) {
            Logger.getLogger(UWMParser.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private static void extractElements(Element root) {
        ...
        List<Element> nList = root.getChildren("course_listing");
...
//            String reg_num = eElement.getChild("reg_num").getText() + genRandomStr();
            ...
            String subj = eElement.getChild("subj").getText();
...
//                    edges.remove(e);
    ...
    private static void saveNodeEdgeFiles() {
        String output_root = "data//output-uw3//";
